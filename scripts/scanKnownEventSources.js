...
function getMonthNumber(dateKey) {
  const value = String(dateKey || '').slice(5, 7)
  const month = Number(value)
  return Number.isFinite(month) ? month : 0
}

function rangeMatchesPreferredMonths(range, preferredMonths = []) {
...
function scoreDateRange(range, seed, targetYear) {
  let score = 0
...
function pickDateRangeForYear(scanResult, seed, targetYear) {
...
function buildCandidate(seed, scanResult, targetYear, todayDateKey) {
  ...
  const extractedRange = getSingleDateRangeForYear(scanResult, seed, targetYear)
